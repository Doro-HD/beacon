...
export async function getRSSItems(
	feedUrl: string
): Promise<Result<Result<RSSItem, string>[], string>> {
	...
	const invokeResult = await invoke<RustResult<RSSItem, string>[]>('get_rss_items', {
		feedUrl,
		feedKind
	});

	// converts all the rust result objects in the array to a result object
	return result.map(invokeResult, (rustResults) =>
		result.ok(rustResults.map((rustResult) => toResult(rustResult)))
	);
}
