...
export const init: ClientInit = async () => {
    ...
    const beaconFileExistsResult  = await fs.exists('data/beacons.json', { baseDir: BaseDirectory.AppData });
    await result.mapAsync(beaconFileExistsResult, async fileExists => {
        if (!fileExists) {
            // initializes the file as an empty array
            const mkdirResult = await fs.writeTextFile('data/beacons.json', '[]', { baseDir: BaseDirectory.AppData });
